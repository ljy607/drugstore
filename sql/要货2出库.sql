...
SELECT  @ckdbh = 'CK16003767',@jsdw = '102',@yhdbh='YH10116000047',@jhdbh='jh16002498'
...
SELECT c.spbh,c.pcbh,c.yxrq,c.jiag,j.lsj,c.chsl,CASE  when m.yhsl>c.chsl THEN c.chsl ELSE m.yhsl END AS sl,c.HWBH,1
FROM t_fdyhzb z
JOIN t_fdyhmxb m ON z.YHDBH = m.YHDBH
JOIN t_chxx c ON c.spbh=m.spbh
JOIN t_jgxx j ON c.spbh = j.SPBH
WHERE z.yhdbh = @yhdbh AND c.hwbh <> 'h01' --AND m.yhsl <= c.chsl
ORDER BY c.spbh
