...
/******************* 过程说明 ******************
  ...
  参数	@bsdbh   报损单编号
	...
	@bsr	 操作人
  结果	生成一个损益单
  变更  增加生产日期的查找 2025年3月13日 09:54:33
************************************************/
ALTER PROCEDURE [dbo].[SP_PPD2BSD] @bsdbh varchar(15),@pddbh varchar(15),@bsr nvarchar(15) AS
begin
	...
	--从存货找价格,生产日期
	update a
	set a.bsjg = isnull(b.jiag,0),a.scrq = b.scrq
	from #t a
	...
	--从出库单找生产日期 2025年3月13日 09:54:12
	update a
	set a.scrq = (SELECT TOP 1 m.scrq FROM T_CKDMXB m join t_ckdzb z on m.ckdbh = z.ckdbh WHERE m.SPBH = A.SPBH order by z.ckrq desc )
	from #t a
	where a.scrq IS NULL
	...
	--从调拨单里找生产日期 2025年3月13日 09:57:28
	update a
	set a.scrq = (SELECT TOP 1 scrq FROM T_dbDMXB WHERE SPBH = A.SPBH AND PCBH = A.PCBH )
	from #t a
	where a.scrq IS NULL
	...
	BEGIN
		...
		values(@bsdbh,getdate(),@bsr,'盘点单'+@pddbh,1)
